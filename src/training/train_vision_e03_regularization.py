"""Phase 5 (E03) training loop: frozen early DenseNet blocks + stronger augmentation."""
...
DEFAULTS: dict[str, Any] = {
    ...
    "checkpoint_path": "models/vision_e03_regularization_best.pt",
    "metrics_output_path": "models/vision_e03_regularization_best_metrics.json",
}
...
def append_experiment_log(repository_root: Path, best_record: dict[str, Any]) -> None:
    """Append one E03 row to the markdown experiment log table.
    ...
    try:
        ...
        row = (
            f"| E03 | {date.today().isoformat()} | 4 | "
            f"DenseNet-121 (ImageNet pretrained) | "
            f"frozen early DenseNet blocks (only denseblock3, denseblock4, classifier trainable) + added rotation/brightness/contrast augmentation, keeping E02's pos_weight class weighting | "
        ...
        LOGGER.info("Appended experiment E03 to %s", log_path)
    except Exception as error:
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and train the E03 regularized vision model."""
    parser = argparse.ArgumentParser(description="Train the Phase 5 (E03) regularized vision model.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[2] / "configs" / "vision_e03_regularization_config.yaml"),
        help="Path to YAML training configuration (default: configs/vision_e03_regularization_config.yaml).",
    )
