"""Phase 6 training loop for the clinical-only MLP baseline."""
...
DEFAULTS: dict[str, Any] = {
    ...
    "checkpoint_path": "models/clinical_baseline_best.pt",
    "metrics_output_path": "models/clinical_baseline_best_metrics.json",
}
...
def compute_pos_weights(dataset: ClinicalDataset) -> torch.Tensor:
    """Compute per-label pos_weight = negatives / positives from the train split.

    Reads the dataset's targets tensor and returns a float32 tensor of
    shape (5,) in LABEL_COLUMNS order, ready to pass as
    ``BCEWithLogitsLoss(pos_weight=...)``. Labels with zero positives -- or
    so few positives that the ratio exceeds the cap -- are capped at
    ``MAX_POS_WEIGHT`` with a warning.
    """
    MAX_POS_WEIGHT = 50.0
    ...
    weights: list[float] = []
    for index, label in enumerate(LABEL_COLUMNS):
        positives = int((targets[:, index] == 1).sum().item())
        negatives = num_samples - positives
        if positives <= 0:
            LOGGER.warning(
                "Label %s has no positive examples in train; capping pos_weight at %.1f.",
                label,
                MAX_POS_WEIGHT,
            )
            weights.append(MAX_POS_WEIGHT)
            continue
        weight = negatives / positives
        if weight > MAX_POS_WEIGHT:
            LOGGER.warning(
                "Label %s pos_weight %.2f exceeds cap; capping at %.1f.",
                label,
                weight,
                MAX_POS_WEIGHT,
            )
    ...
    LOGGER.info("Per-label pos_weight (negatives/positives from train split):")
    for label, weight in zip(LABEL_COLUMNS, weights):
        LOGGER.info("  %s: %.4f", label, weight)
    return torch.tensor(weights, dtype=torch.float32)


def append_experiment_log(repository_root: Path, best_record: dict[str, Any]) -> None:
    """Append one E06 row to the markdown experiment log table.
    ...
    try:
        ...
        row = (
            f"| E06 | {date.today().isoformat()} | 6 | "
            f"Clinical MLP (29 features, 64-32-5) | "
            f"clinical-only baseline: age, BMI (median-imputed + missingness flag), "
            f"sex/race/ethnicity/insurance_type (one-hot), no images | "
        ...
        LOGGER.info("Appended experiment E06 to %s", log_path)
    except Exception as error:
...
def train_model(config_path: str | Path) -> dict[str, Any]:
    """Train the clinical baseline, checkpoint on mean val AUROC, and report operating points."""
    ...
    train_dataset = ClinicalDataset(manifest_path, split="train")
    val_dataset = ClinicalDataset(manifest_path, split="val")
    ...
    LOGGER.info(
        "Loaded datasets: train=%d, val=%d, feature_dim=%d, batch_size=%d",
        len(train_dataset),
        len(val_dataset),
    ...
    model = build_clinical_baseline(input_dim=train_dataset.feature_dim).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        LOGGER.info(
            "Epoch %d/%d | train_loss=%.4f val_loss=%.4f mean_val_auroc=%.4f%s | lr=%.6f",
            epoch,
            max_epochs,
            train_loss,
            val_loss,
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and train the Phase 6 clinical baseline."""
    parser = argparse.ArgumentParser(description="Train the Phase 6 clinical baseline.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[2] / "configs" / "clinical_baseline_config.yaml"),
        help="Path to YAML training configuration (default: configs/clinical_baseline_config.yaml).",
    )
