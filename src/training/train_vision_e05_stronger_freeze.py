"""Phase 5 (E05) training loop: E04 setup plus stronger layer freeze (denseblock4+ only)."""
...
DEFAULTS: dict[str, Any] = {
    ...
    "checkpoint_path": "models/vision_e05_stronger_freeze_best.pt",
    "metrics_output_path": "models/vision_e05_stronger_freeze_best_metrics.json",
}
...
def compute_pos_weights(dataset: ChestXrayDataset) -> torch.Tensor:
    """Compute per-label pos_weight = negatives / positives from the train split.

    Reads the dataset's underlying dataframe and returns a float32 tensor of
    ...
    for label in LABEL_COLUMNS:
        positives = int((dataset.records[label] == 1).sum())
...
def append_experiment_log(repository_root: Path, best_record: dict[str, Any]) -> None:
    """Append one E05 row to the markdown experiment log table.
    ...
    try:
        ...
        row = (
            f"| E05 | {date.today().isoformat()} | 4 | "
            f"DenseNet-121 (ImageNet pretrained) | "
            f"stronger freeze (only denseblock4, norm5, classifier trainable -- froze denseblock3 in addition to E04's frozen blocks), otherwise identical to E04 | "
        ...
        LOGGER.info("Appended experiment E05 to %s", log_path)
    except Exception as error:
...
def train_model(config_path: str | Path) -> dict[str, Any]:
    ...
    train_dataset = ChestXrayDataset(
        manifest_path, split="train", transform_builder=build_transform_e03
    )
    ...
    model = build_vision_baseline_frozen(unfreeze_from_block="denseblock4").to(device)
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and train the E05 stronger-freeze vision model."""
    parser = argparse.ArgumentParser(description="Train the Phase 5 (E05) stronger-freeze vision model.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[2] / "configs" / "vision_e05_stronger_freeze_config.yaml"),
        help="Path to YAML training configuration (default: configs/vision_e05_stronger_freeze_config.yaml).",
    )
