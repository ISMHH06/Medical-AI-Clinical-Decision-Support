"""Phase 5 (E02) training loop: DenseNet-121 baseline plus per-label class weights."""
...
DEFAULTS: dict[str, Any] = {
    ...
    "checkpoint_path": "models/vision_e02_class_weights_best.pt",
    "metrics_output_path": "models/vision_e02_class_weights_best_metrics.json",
}
...
def append_experiment_log(repository_root: Path, best_record: dict[str, Any]) -> None:
    """Append one E02 row to the markdown experiment log table.
    ...
    try:
        ...
        row = (
            f"| E02 | {date.today().isoformat()} | 4 | "
            f"DenseNet-121 (ImageNet pretrained) | "
            f"class-imbalance weighting via per-label pos_weight (auto-computed from train split), otherwise identical to E01 | "
        ...
        LOGGER.info("Appended experiment E02 to %s", log_path)
    except Exception as error:
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and train the E02 class-weighted vision model."""
    parser = argparse.ArgumentParser(description="Train the Phase 5 (E02) class-weighted vision model.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[2] / "configs" / "vision_e02_class_weights_config.yaml"),
        help="Path to YAML training configuration (default: configs/vision_e02_class_weights_config.yaml).",
    )
