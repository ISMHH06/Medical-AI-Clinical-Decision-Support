"""Phase 5 (E04) training loop: E03 setup plus ReduceLROnPlateau LR scheduling."""
...
DEFAULTS: dict[str, Any] = {
    ...
    "checkpoint_path": "models/vision_e04_lr_schedule_best.pt",
    "metrics_output_path": "models/vision_e04_lr_schedule_best_metrics.json",
}
...
def append_experiment_log(repository_root: Path, best_record: dict[str, Any]) -> None:
    """Append one E04 row to the markdown experiment log table.
    ...
    try:
        ...
        row = (
            f"| E04 | {date.today().isoformat()} | 4 | "
            f"DenseNet-121 (ImageNet pretrained) | "
            f"added ReduceLROnPlateau LR scheduler (factor=0.5, patience=2, monitoring val AUROC), max_epochs increased to 20, otherwise identical to E03 | "
        ...
        LOGGER.info("Appended experiment E04 to %s", log_path)
    except Exception as error:
...
def train_model(config_path: str | Path) -> dict[str, Any]:
    ...
    model = build_vision_baseline_frozen().to(device)
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and train the E04 LR-scheduled vision model."""
    parser = argparse.ArgumentParser(description="Train the Phase 5 (E04) LR-scheduled vision model.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[2] / "configs" / "vision_e04_lr_schedule_config.yaml"),
        help="Path to YAML training configuration (default: configs/vision_e04_lr_schedule_config.yaml).",
    )
